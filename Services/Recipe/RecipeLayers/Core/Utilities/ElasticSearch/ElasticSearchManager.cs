    ...
    public class ElasticSearchManager : IElasticSearch
    {
        ...
        public async Task<IResult> CreateNewIndexAsync(IndexModel indexModel)
        {
            ...
            if (elasticClient.Indices.Exists(indexModel.IndexName).Exists)
                return new Results.Concrete.Result(success: false, message: "Index already exists");
            ...
            return new Results.Concrete.Result(success: response.IsValid,
        ...
        public async Task<List<ElasticSearchGetModel<T>>> GetByTerms<T>(SearchByTermsParameters termsParameters) where T: class
        {
            var elasticClient = GetElasticClient(termsParameters.IndexName);
            var searchResponse = await elasticClient.SearchAsync<T>(s => s
                        .Index(termsParameters.IndexName)
                        .From(termsParameters.From)
                        .Size(termsParameters.Size)
                        .MinScore(0.5)
                        .Query(q => q.Terms(
                            m => m.Field(termsParameters.FieldName)
                            .Terms(termsParameters.Terms)
                            )
                         )
                       );
        ...
        public IResult InsertManyWithBulkAll<T>(string indexName, T [] items) where T:class
        {
            var elasticClient = GetElasticClient(indexName);
            //var response =  elasticClient.BullkAll(a =>
            //                     a.Index(indexName)
            //                     .IndexMany(items));

            var response =  elasticClient.BulkAll(items, b => b
                    .Index(indexName)
                    .BackOffTime("30s")
                    .BackOffRetries(2)
                    .RefreshOnCompleted()
            ...
            return new Results.Concrete.Result(success:true,
                                     message:  "Success" );
        }
